...
class QuizesController < ApplicationController
  ...
  def show
    ...
    challenge_detail = Challenges.find_by_id(current_access_token, params[:challenge_id])[0]
    ...
    redirect_to challenge_path(params[:challenge_id]) unless challenge_participation_status[:status].eql?('Registered')
  ...
  private
    ...
    def challenge_participation_status
      if signed_in?      
        participation = Challenges.participant_status(current_access_token, current_user.username, params[:challenge_id])[0]
