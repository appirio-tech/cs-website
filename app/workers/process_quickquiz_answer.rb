class ProcessQuickQuizAnswer
  ...
  def self.perform(access_token, username, params)
    ...
    if params.has_key?('answer')
      ...
      Rails.logger.info "[Resque]==== QuickQuiz submission for #{username} and question #{params["question_id"]}. Answer being evaluted: #{params["correct"]}"
    end
    ...
    Rails.logger.info "[Resque]==== QuickQuiz submission for #{username} and question #{params["question_id"]} and type is #{params['p']}. Results: #{results}"
